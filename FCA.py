    ...
    while cur_concept:
        for x1,y1 in cur_concept:
            for x2,y2 in cur_concept:
                ...
                extent = find_extent(y, full_objects, C)
                intent = find_intent(full_objects, extent, C)
                intent_frozen = frozenset(intent)
                extent_frozen = frozenset(extent)
                if (extent_frozen, intent_frozen) not in conceptset:
                    conceptset.add((extent_frozen, intent_frozen))
                    next_concept.add((extent_frozen, intent_frozen))
...
def BasicConcept(C):
    ...
    for j in range(len(C)):
        extent = find_extent({j}, full_objects, C)
        intent = find_intent(full_objects, extent, C)
        extent_frozen = frozenset(extent)
        intent_frozen = frozenset(intent)
        if (extent_frozen,intent_frozen) not in conceptset:
